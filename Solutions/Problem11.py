...
for i in range(1, 21):
    grid.append([int(n) for n in buf.readline().split()])


# find greatest value in rows
def horizontal_greatest(grid):
    g = 0
    for row in grid:
...
# find greatest value in columns
def vertical_greatest(grid):
    vert_grid = zip(*grid) # transpose the grid
    return horizontal_greatest(vert_grid)

# diagonal down (NW to SE) computations
# TODO

# diagonal up (SW to NE) computations
# TODO
